...
export const WaitingRoomPage: React.FC<WaitingRoomPageProps> = ({
  ...
  const isCountdown = eventStatus === 'COUNTDOWN' || (countdown !== null && countdown > 0);
  const isLive = eventStatus === 'LIVE';
  const isPaused = eventStatus === 'PAUSED';
  const isWaiting = eventStatus === 'WAITING' || !gameState?.event;
        ...
        <div className="tech-card rounded-2xl p-8 sm:p-10 border border-cyan-500/30 shadow-2xl">
          ...
          <div className="bg-navy-950/90 rounded-xl p-5 border border-slate-800 text-left space-y-3.5 mb-6">
            <div className="flex items-center justify-between font-mono text-xs">
              ...
              <span className={`font-bold flex items-center gap-1.5 px-2.5 py-1 rounded border ${
                isLive
              ...
              {isLive ? (
                <div>
                  ...
                  <p className="text-xs text-slate-300 leading-relaxed font-sans mb-3">
                    The coordinator has started the competition. Transitioning to your live game arena...
                  </p>
                  {onEnterArena && (
                    <button
                      onClick={onEnterArena}
                      className="w-full py-2.5 px-4 rounded bg-cyan-400 hover:bg-cyan-300 text-navy-950 font-sans font-bold text-xs uppercase tracking-wider transition shadow-cyan-glow flex items-center justify-center gap-2"
                    >
                ...
                <div>
                  <span className="text-xs font-mono font-bold text-amber-400 uppercase tracking-wider block mb-1">
                    {isCountdown ? 'COUNTDOWN IN PROGRESS' : 'EVENT NOT STARTED'}
                  </span>
                  <p className="text-xs text-slate-300 leading-relaxed font-sans">
                    {isCountdown
                      ? 'The coordinator has triggered the start. The arena will launch in seconds.'
          ...
          <div className="text-left bg-cyan-950/20 border border-cyan-500/20 rounded-lg p-4 mb-2">
            ...
            <ul className="text-xs text-slate-400 space-y-1.5 font-sans leading-relaxed">
              <li>• The event will begin automatically when the coordinator broadcasts the start.</li>
