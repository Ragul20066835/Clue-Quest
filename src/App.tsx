...
export const App: React.FC = () => {
  ...
  const [isInitializing, setIsInitializing] = useState<boolean>(() => {
    return !sessionStorage.getItem('cq_initialized');
  });

  const handleBootComplete = React.useCallback(() => {
    sessionStorage.setItem('cq_initialized', 'true');
    ...
    if (user) {
      if (user.role === 'ADMIN') {
        ...
        if (gameState?.session?.status === 'COMPLETED') {
          if (currentTab !== 'results' && currentTab !== 'leaderboard' && currentTab !== 'help') {
            setCurrentTab('results');
          }
        } else if (gameState?.event?.status === 'LIVE' || gameState?.event?.status === 'PAUSED') {
          if (currentTab === 'waiting' || currentTab === 'login' || currentTab === 'admin_login' || currentTab === 'home') {
            setCurrentTab('game');
          }
        } else if (gameState?.event?.status === 'WAITING' || !gameState?.event) {
          if (currentTab === 'game' || currentTab === 'login' || currentTab === 'admin_login' || currentTab === 'home') {
            setCurrentTab('waiting');
          }
        }
      }
    }
  }, [user, gameState?.event?.status, gameState?.session?.status, currentTab]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-navy-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-navy-950">
      ...
      {isInitializing && (
        <ClueQuestInitializer onComplete={handleBootComplete} />
      )}
      ...
      <div className="flex-1">
        ...
        {/* Player Game Flow */}
        {currentTab === 'waiting' && (
          <WaitingRoomPage onEnterArena={() => setCurrentTab('game')} />
        )}
        {currentTab === 'game' && (
