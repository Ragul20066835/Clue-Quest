...
export const ClueQuestInitializer: React.FC<ClueQuestInitializerProps> = ({ onComplete }) => {
  ...
  // 3. Skip on Escape or any key / click
  useEffect(() => {
  ...
  // 5. Snappy status message & progress animation cycle (~1.2s total)
  useEffect(() => {
    const stepDurations = [180, 180, 180, 180, 180, 180, 250];
  ...
  return (
    <div
      ...
      aria-live="polite"
      onClick={finishBoot}
      className={`fixed inset-0 w-full h-screen z-[9999] flex flex-col justify-between items-center bg-[#020711] text-[#F5F7FA] select-none cursor-pointer transition-opacity duration-500 ease-out overflow-hidden ${
      ...
      <header className="relative z-10 w-full max-w-7xl mx-auto px-6 pt-6 flex items-center justify-between font-mono text-[10px] sm:text-xs tracking-[0.2em] text-[#7C91A8]">
        ...
        <div>
          <button
            ...
            className="px-2.5 py-1 rounded bg-slate-800/80 hover:bg-cyan-500/20 text-cyan-300 border border-slate-700 hover:border-cyan-500/40 text-[10px] font-mono tracking-wider transition-all duration-150"
          >
            SKIP [ESC / CLICK]
          </button>
        </div>
